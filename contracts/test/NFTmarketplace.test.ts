...
//         newNft = await musicalNft.connect(addr1)
...
//         await musicalNft.connect(addr1).setApprovalForAll(marketplaceContract.target, true)
//         const itemId = await marketplaceContract.connect(addr1)
...
//         expect(item._nft).to.equal(newNft.address);
